# facs_calc_log2fc() + facs_calc_diff() Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `facs_calc_log2fc()` and `facs_calc_diff()` to `R/facs_calc.R` — two T-cell restimulation normalizers that compare each stimulated condition's `ref_pop`-normalized proportion against an unstimulated reference condition, per mouse/tissue/population: a log2 fold-change and a plain difference, respectively. Replaces a manually-written draft that hardcoded stim condition names, hardcoded a `group` passthrough column, and had a pseudocount default (`0`) that contradicted its own comment.

**Architecture:** One new unexported helper, `calc_restim_proportions_()`, appended to the existing `R/facs_calc.R` (alongside `facs_calc_pct_of()`/`facs_calc_count_per_g()`), shared by two new thin exported functions. The helper does everything through the `ref_pop`-normalized-proportion wide pivot (ambiguity/missing checks, pseudocount, passthrough-column auto-detection, missing-`ref_level` check); each public function applies its own final comparison (`log2(x / ref)` vs. `x - ref`) and its own `pivot_longer()` (value column `log2fc` vs. `diff`). Neither public function appends rows to `data` and returns it, unlike their siblings — their output grain (one row per `mouse_ID` x `tissue` x `population` x non-reference restimulation level) collapses the restimulation dimension via comparison, so each returns a new, differently-shaped tibble.
...
- Namespacing: every external function call inside an exported (or shared unexported) function body must be `pkg::fn()` — no bare `dplyr`/`tidyr`/`glue`/`purrr` calls. Avoid `\(x)` backslash-lambda syntax (not used elsewhere in this codebase) — use `function(x) ...` instead.
- Deselection uses `dplyr::select(!col)`, never `-col`.
- `facs_calc_log2fc()`/`facs_calc_diff()` each return a **new** tibble (not `data` with rows appended, unlike `facs_calc_pct_of()`/`facs_calc_count_per_g()`) — do not wrap in `dplyr::bind_rows(data, ...)`.
- Both functions return visibly (primary output, not a side effect) — do not wrap in `invisible()`.
- File/function naming: `domain_verb.R` / `domain_verb_qualifier()`, domain prefix `facs_`. `calc_restim_proportions_()` is unexported (trailing underscore, matches `resolve_var_()`/`is_constant_` convention) — no `domain_` prefix needed for internal helpers.
- Both exported functions need `@param`, `@returns`, `@export`; `@examples` uses `\dontrun{}` since both require realistic multi-column input. `calc_restim_proportions_()` is unexported — no roxygen block, just a one-line `#` comment noting it's shared by both public functions (matches `resolve_var_()` precedent).
...
## Task 1: `calc_restim_proportions_()` + `facs_calc_log2fc()` — core implementation + happy-path/error tests
...
**Interfaces:**
- Produces: `calc_restim_proportions_(data, ref_pop, restim_col, ref_level, pseudocount)` — unexported. Returns a wide tibble (`mouse_ID`, `tissue`, `population`, `<passthrough cols>`, one column per distinct `restim_col` value holding that condition's proportion) with an `"id_cols"` attribute (character vector: `c("mouse_ID", "tissue", "population", <passthrough cols>)`) attached via `attr(wide, "id_cols")`, for callers to compute `stim_cols <- setdiff(names(wide), c(attr(wide, "id_cols"), ref_level))`.
...
- [ ] **Step 3: Implement `calc_restim_proportions_()` and `facs_calc_log2fc()`**
...
**Interfaces:**
- Consumes: `facs_calc_log2fc()` from Task 1 (already implements the full logic via `calc_restim_proportions_()` — this task's job is proving every remaining branch behaves as specified).
...
Run: `Rscript -e 'devtools::load_all(); testthat::test_file("tests/testthat/test-facs_calc.R")'`
Expected: PASS, 26 tests total, 0 failures — Task 1's implementation already covers these branches. If any of these 6 new tests fail, fix `facs_calc_log2fc()`/`calc_restim_proportions_()` in `R/facs_calc.R` (do not weaken the test) before proceeding.
...
## Task 3: `facs_calc_diff()` — subtraction-based comparison
...
**Interfaces:**
- Consumes: `calc_restim_proportions_()` from Task 1 (unchanged — already implements every shared branch: `restim_col` validation, `ref_pop` ambiguity/missing checks, passthrough-column detection, missing-`ref_level` check).
- Produces: `facs_calc_diff(data, ref_pop, restim_col = "restimulation", ref_level = "unstim", pseudocount = 0)` — exported. Same shape as `facs_calc_log2fc()`'s input; returns a new tibble: `mouse_ID`, `tissue`, `population`, `<passthrough cols>`, `restim_col` (name reused), `diff`.
...
Run: `Rscript -e 'devtools::load_all(); testthat::test_file("tests/testthat/test-facs_calc.R")'`
Expected: FAIL with `could not find function "facs_calc_diff"`.

- [ ] **Step 3: Implement `facs_calc_diff()`**
...
```bash
Rscript -e 'devtools::document()'
git add R/facs_calc.R tests/testthat/test-facs_calc.R man/facs_calc_diff.Rd NAMESPACE
git commit -m "feat: add facs_calc_diff() for subtraction-based restimulation comparison"
```
...
```
  - All `calc_restim_proportions_`/`facs_calc_log2fc`/`facs_calc_diff` variable-binding notes (`population`, `metric`, `value`, `mouse_ID`, `tissue`, `ref_count`, `n`) — bare column names inside dplyr verbs, same false-positive pattern as `facs_calc_pct_of`/`facs_calc_count_per_g` above. The internal `is_constant_` closure's `mouse_ID`/`tissue` references (bare inside `dplyr::group_by()`) are also flagged for the same reason. `calc_restim_proportions_` is unexported (shared by both public functions), so its notes are attributed to `R/facs_calc.R` as a whole rather than one function.
```
...
Run: `Rscript -e 'devtools::check()'`
Expected: 0 errors, 0 warnings (NOTEs limited to the pre-existing ones already listed in CLAUDE.md, plus the new `calc_restim_proportions_`/`facs_calc_log2fc`/`facs_calc_diff` variable-binding NOTE now documented in Step 3).

- [ ] **Step 5: Commit**

```bash
git add CLAUDE.md
git commit -m "docs: note facs_calc_log2fc/facs_calc_diff variable-binding NOTEs in CLAUDE.md"
```
...
- **Spec coverage:** `calc_restim_proportions_()` (restim_col validation, ref_pop ambiguity/missing checks, pseudocount, passthrough-column detection, missing-ref_level check) plus `facs_calc_log2fc()`'s log2 comparison — Task 1. Remaining `facs_calc_log2fc()` branches (multi-condition, passthrough keep/drop, custom column names) — Task 2. `facs_calc_diff()`'s subtraction comparison, default `pseudocount = 0`, and shared-helper wiring (ambiguity error, missing-ref_level warning) — Task 3. `devtools::check()` verification and CLAUDE.md housekeeping — Task 4. All design-doc sections (Internal helper, Public API for both functions, Error handling, Testing) covered. Per the design doc, no end-to-end fixture test is included for either function — `tests/fixtures/minimal.wsp` doesn't carry `mouse_ID`/`tissue`/`restimulation` keywords, consistent with `facs_calc_pct_of()` also being unit-tested only for that reason. `calc_restim_proportions_()` itself is not directly tested, matching the existing `resolve_var_()` precedent (exercised indirectly through its callers).
- **Type consistency:** `calc_restim_proportions_(data, ref_pop, restim_col, ref_level, pseudocount)`'s signature and its `"id_cols"` attribute contract are used identically by both `facs_calc_log2fc()` (Task 1) and `facs_calc_diff()` (Task 3). `facs_calc_log2fc()`'s and `facs_calc_diff()`'s public signatures (`data, ref_pop, restim_col, ref_level, pseudocount`) match across implementation and every test call site.
