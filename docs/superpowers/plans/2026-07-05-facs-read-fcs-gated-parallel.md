...
**Interfaces:**
- Produces: `facs_read_fcs_gated(..., workers = 1L)` — new argument. `workers == 1` behaves exactly as after Task 1. `workers > 1` dispatches per-sample work across forked processes via `parallel::mclapply()`.
- Modifies: `read_one_sample_solo_()` now returns `list(data = tibble, warnings = character())` instead of a bare tibble — necessary because `parallel::mclapply()` forks do not propagate `warning()` calls back to the parent process as real conditions; they must be captured in the child and re-issued in the parent.
...
- [ ] **Step 3: Wrap `read_one_sample_solo_()`'s return value to capture warnings**

In `R/facs_read_fcs.R`, replace the full body of `read_one_sample_solo_()` (everything from the `gs <-` line through the final `tibble::add_column(...)` call) with:
...
Note the `if (is.null(gated)) { ... } else { ... }` restructuring (replacing the old early `return(tibble::tibble())`): a bare `return()` inside the `withCallingHandlers({...})` block would exit `read_one_sample_solo_()` immediately, skipping the final `list(data =, warnings =)` wrap — the if/else must produce the tibble as the block's value instead.
...
```r
  ...
  results <- if (workers > 1L) {
    parallel::mclapply(seq_along(sample_names), worker_fn, mc.cores = workers, mc.set.seed = TRUE)
  } else {
    purrr::map(seq_along(sample_names), worker_fn)
  }

  errored <- purrr::keep(results, ~ inherits(.x, "try-error"))
  if (length(errored) > 0L) {
    stop(conditionMessage(attr(errored[[1]], "condition")), call. = FALSE)
  }
...
across forked processes via parallel::mclapply(), since profiling
showed GatingSet construction -- not the post-build extraction step --
dominates runtime and scales linearly per sample. Warnings are
captured per-worker and re-issued in the parent process (mclapply
forks don't propagate warning() conditions back), and per-sample
downsampling now uses a seed derived from each sample's position
rather than one shared sequential RNG stream, so results no longer
depend on execution/fork order."
```
...
#' @param workers integer; number of samples to process in parallel via
#'   \code{parallel::mclapply()}. Default \code{1} processes samples
#'   sequentially. Fork-based, so on Windows this silently falls back to
#'   sequential execution regardless of the value passed.
```
