...
analysis_summary_table <- function(data, group_col = "group",
  ...
  summary_tbl <- function(input) {
    numeric_cols <- names(input)[vapply(input, is.numeric, logical(1))]
    input |>
      gtsummary::tbl_summary(
        include   = dplyr::all_of(numeric_cols),
        by        = "group",
        statistic = list(gtsummary::all_continuous() ~ "{median} ({p25}, {p75})"),
        digits    = gtsummary::all_continuous() ~ 2,
        missing   = "no"
      ) |>
      gtsummary::add_p(test = gtsummary::all_continuous() ~ "kruskal.test") %>% # %>% required for gtsummary method dispatch
  ...
  if (!is.null(tissue_col)) {
    ...
    filter_fun <- function(dat, tis) {
      dplyr::filter(dat, .data[[tissue_col]] == tis)
    }

    ## mapping function to create subsetted list by tissue
    tissue_data <- purrr::map(
      .x = tissues,
      .f = ~ filter_fun(dat = data, tis = .x)
    ) |>
      purrr::set_names(tissues)
    ...
    summary_fun <- function(dat, tis) {
      dat[[tis]] |>
        summary_tbl() |>
        gt::tab_caption(caption = gt::md(paste0("**Descriptive Statistics for ", tis, "**")))
    }

    ## mapping function to map summary function over all tissues
    desc_tables <- purrr::map(
      .x = tissues,
...
analysis_posthoc_tables <- function(data, group_col = "group",
  ...
  posthoc_fun <- function(dat, tis, var, group_col) {
    ...
    form <- stats::reformulate(group_col, response = as.name(var))
    ...
    if (kw_p < 0.05) {
      dunn <- rstatix::dunn_test(
        ...
        gt::fmt_number(columns = c("p.adj", "statistic", "p"), decimals = 4) |>
        gt::tab_style(
          style     = gt::cell_fill(color = "lightgreen"),
          locations = gt::cells_body(columns = c("p.adj.signif", "p.adj"), rows = p.adj < 0.05)
        ) |>
        gt::cols_hide(".y.")
    }
  ...
  if (!is.null(tissue_col)) {
    ...
    all_pops_fun <- function(dat, tis, group_col) {
      ...
      data <- dplyr::filter(dat, .data[[tissue_col]] == tis)
    ...
    # apply analysis function for all pops to every tissue
    tab <- purrr::map(
      .x = tissues,
  ...
  ### if tissue column is NULL no subsetting is needed and summary table is immediately created
    ...
    all_pops_fun <- function(dat, group_col) {
      ...
      ## extract names of numeric variable columns
      cols <- names(dat)[vapply(dat, is.numeric, logical(1))]

      ## apply post-hoc function to all numeric cols
      purrr::map(
        .x = cols,
        .f = ~ posthoc_fun(dat, "", var = .x, group_col = group_col)
      ) |>
        purrr::set_names(cols)
    }
